...
import { doc, setDoc, getDoc, collection, addDoc, serverTimestamp, query, where, getDocs, orderBy, Timestamp, onSnapshot, deleteDoc } from 'firebase/firestore';
    ...
    <div className="flex-1 flex justify-start items-center gap-4">
      <Link to="/" className="flex items-center gap-2 md:gap-3 relative group cursor-pointer ml-0 md:-ml-4 transition-all">
        ...
        <div className="flex flex-col -space-y-1">
          <h1 className="text-lg md:text-2xl font-logo font-extrabold tracking-[-0.05em] text-white relative z-10 group-hover:accent-glow transition-all duration-500">
            ORIGINAL
          </h1>
          <span className="text-[0.35rem] md:text-[0.45rem] font-black tracking-[0.6em] text-accent uppercase opacity-80 pl-1">Archives</span>
        </div>
      </Link>
    </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
          <div className="space-y-3">
             ...
             <h2 className="text-2xl md:text-7xl font-logo font-extrabold tracking-[-0.05em] uppercase italic leading-[0.9]">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 flex flex-col lg:flex-row gap-8 lg:gap-16 mb-8">
        ...
        <div className="w-full lg:w-[35%] flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 order-2 relative">
            ...
            <div 
              ref={mobileSliderRef}
              className="md:hidden flex overflow-x-auto snap-x snap-mandatory scrollbar-hide aspect-[4/5] rounded-2xl border border-white/5 bg-secondary-bg touch-pan-x"
              onScroll={(e) => {
            ...
            >
              {product.images.map((img, idx) => (
                <div key={idx} className="min-w-full h-full snap-start flex-shrink-0">
                  <img 
                    src={img} 
                    className="w-full h-full object-cover" 
                    alt="" 
            ...
            {/* Desktop Static Image with Fade */}
            <div className="hidden md:block aspect-[4/5] max-h-[65vh] rounded-2xl overflow-hidden bg-secondary-bg border border-white/5 relative group">
              <AnimatePresence mode="wait">
                <motion.img 
                  key={activeImageIndex}
                  initial={{ opacity: 0 }}
            ...
            {/* Mobile Dots Indicators - now integrated here */}
...
const CheckoutPage = ({ 
  ...
  if (cart.length === 0 && !orderSuccess) {
    return (
      <div className="min-h-screen bg-dark-bg pt-32 pb-20 px-6 flex flex-col items-center justify-center text-center">
        <div className="w-20 h-20 border border-white/10 rounded-full flex items-center justify-center mb-8">
          <ShoppingBag size={32} className="text-white/20" />
        </div>
        <h2 className="text-3xl font-logo font-extrabold tracking-tight uppercase italic mb-4">Your Cart is Empty</h2>
        <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-10">Return to archives to begin your collection</p>
        <Link to="/" className="bg-accent text-black px-10 py-4 font-black tracking-widest text-[0.6rem] uppercase hover:brightness-110 transition-all">
          EXPLORE COLLECTIONS
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-dark-bg pt-24 pb-20 px-4 md:px-10 max-w-7xl mx-auto">
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3 text-accent text-[0.6rem] font-black tracking-[0.4em] uppercase mb-2">
            <span className="w-8 h-[1px] bg-accent/30"></span>
            Transaction // Secure
          </div>
          <h1 className="text-3xl md:text-6xl font-logo font-extrabold tracking-[-0.05em] uppercase italic leading-none">
            {orderSuccess ? "Order" : "Secure"} <span className="text-outline">{orderSuccess ? "Placed" : "Checkout"}</span>
          </h1>
        </div>
        <button 
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-white/40 hover:text-white text-[0.6rem] font-black tracking-widest uppercase transition-colors"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-7">
          <div className="bg-secondary-bg border border-white/10 rounded-2xl p-6 md:p-10 shadow-2xl overflow-hidden relative">
            <AnimatePresence mode="wait">
              {orderSuccess ? (
                <motion.div 
                  key="success"
                ...
                >
                  <div className="w-24 h-24 bg-accent/10 border border-accent/20 rounded-full flex items-center justify-center mx-auto mb-8 shadow-[0_0_50px_rgba(242,125,38,0.1)]">
                    <CheckCircle2 size={48} className="text-accent" />
                  </div>
                  <div className="space-y-4">
                    <h2 className="text-2xl md:text-4xl font-logo font-extrabold tracking-tight uppercase italic">Order Confirmed</h2>
                    <p className="text-white/40 text-[0.7rem] font-bold tracking-[0.2em] uppercase leading-relaxed max-w-sm mx-auto">
                      Your order has been authorized and dispatched to our logistics center. A confirmation manifest has been sent to your registered mail.
                    </p>
                  </div>
                  <div className="pt-8 flex flex-col items-center gap-4">
                    <Link to="/profile" className="bg-white text-black px-12 py-5 font-black tracking-[0.3em] text-[0.6rem] hover:bg-accent hover:text-black transition-all uppercase w-full md:w-auto">
                      VIEW ORDER STATUS
                    </Link>
                    <Link to="/" className="text-white/40 hover:text-white font-black tracking-widest text-[0.55rem] uppercase transition-all">
                      CONTINUE EXPLORING
                    </Link>
                  </div>
                </motion.div>
                  ...
                  <form onSubmit={onPlaceOrder} className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        ...
                        <input name="name" required placeholder="FULL NAME" className="w-full bg-white/5 border border-white/10 rounded-lg px-5 py-5 text-xs font-bold outline-none focus:border-accent transition-colors placeholder:opacity-20" />
                      </div>
                      <div className="space-y-3">
                        <label className="text-[0.55rem] font-black tracking-[0.4em] text-gray-500 uppercase">Communication // Mail</label>
                        <input name="email" type="email" required placeholder="EMAIL@ORIGINAL.LABS" className="w-full bg-white/5 border border-white/10 rounded-lg px-5 py-5 text-xs font-bold outline-none focus:border-accent transition-colors placeholder:opacity-20" />
                      </div>
                    </div>
                    <div className="space-y-3">
                      <label className="text-[0.55rem] font-black tracking-[0.4em] text-gray-500 uppercase">Logistics // Street Address</label>
                      <input name="address1" required placeholder="HOUSE NO., STREET, AREA" className="w-full bg-white/5 border border-white/10 rounded-lg px-5 py-5 text-xs font-bold outline-none focus:border-accent transition-colors placeholder:opacity-20" />
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        <label className="text-[0.55rem] font-black tracking-[0.4em] text-gray-500 uppercase">Logistics // City</label>
                        <input name="city" required placeholder="CITY" className="w-full bg-white/5 border border-white/10 rounded-lg px-5 py-5 text-xs font-bold outline-none focus:border-accent transition-colors placeholder:opacity-20" />
                      </div>
                      <div className="space-y-3">
                        <label className="text-[0.55rem] font-black tracking-[0.4em] text-gray-500 uppercase">Logistics // Pincode</label>
                        <input name="pincode" required placeholder="PINCODE" className="w-full bg-white/5 border border-white/10 rounded-lg px-5 py-5 text-xs font-bold outline-none focus:border-accent transition-colors placeholder:opacity-20" />
                      </div>
                    </div>

                    <div className="space-y-3">
                      <label className="text-[0.55rem] font-black tracking-[0.4em] text-gray-500 uppercase">Logistics // State</label>
                      <input name="state" required placeholder="STATE / PROVINCE" className="w-full bg-white/5 border border-white/10 rounded-lg px-5 py-5 text-xs font-bold outline-none focus:border-accent transition-colors placeholder:opacity-20" />
                    </div>
                    ...
                    <div className="pt-10 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-8">
                      <div className="text-center md:text-left">
                        ...
                        <p className="text-4xl font-black italic tracking-tighter">₹{subtotal}.00</p>
                      </div>
                      <button 
                        type="submit"
                        className="w-full md:w-auto bg-accent text-black px-10 md:px-16 py-4 md:py-6 font-black tracking-[0.3em] text-[0.6rem] md:text-[0.7rem] hover:brightness-110 active:scale-95 transition-all shadow-[0_10px_30px_rgba(242,125,38,0.2)]"
                      >
                        REVIEW ORDER
                      </button>
                    </div>
                ...
                >
                  <div className="bg-white/5 border border-white/10 rounded-xl p-8 space-y-6">
                    <div className="flex justify-between items-start">
                      <h3 className="text-[0.6rem] font-black tracking-[0.3em] uppercase text-accent">Delivery Parameters</h3>
                      <button onClick={() => setIsConfirming(false)} className="text-[0.5rem] font-black tracking-widest text-white/30 hover:text-white uppercase">Edit Details</button>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-1 text-[0.7rem] font-bold uppercase tracking-widest leading-relaxed">
                        <p className="text-white/20 text-[0.5rem] mb-1">Recipient</p>
                        {pendingShippingDetails?.name}
                        <p className="text-white/40 lowercase">{pendingShippingDetails?.email}</p>
                      </div>
                      <div className="space-y-1 text-[0.7rem] font-bold uppercase tracking-widest leading-relaxed">
                        <p className="text-white/20 text-[0.5rem] mb-1">Destination</p>
                  ...
                  <div className="space-y-6">
                    <h3 className="text-[0.6rem] font-black tracking-[0.3em] uppercase text-accent">Payment Protocol</h3>
                    <div className="bg-white/5 border border-white/10 rounded-xl p-6 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center font-black">₹</div>
                        <div>
                          <p className="text-[0.65rem] font-black tracking-widest uppercase mb-0.5">Cash on Delivery</p>
                          <p className="text-[0.5rem] font-bold text-white/30 uppercase">Settlement upon physical acquisition</p>
                        </div>
                      </div>
                  ...
                  <button 
                    ...
                    className="w-full bg-white text-black py-4 md:py-6 font-black tracking-[0.4em] text-[0.6rem] md:text-[0.7rem] hover:bg-accent transition-all active:scale-95 shadow-[0_15px_40px_rgba(255,255,255,0.05)] uppercase"
                  >
                    CONFIRM & PLACE ORDER
                  </button>
                </motion.div>
        ...
        <div className="lg:col-span-5">
          <div className="sticky top-28 space-y-8">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-8">
              <h2 className="text-xl font-logo font-extrabold tracking-[-0.05em] uppercase italic">
                Cart <span className="text-outline">Contents</span>
              </h2>
              
              <div className="space-y-6 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
                {cart.map((item) => (
                  <div key={item.id} className="flex gap-4">
                    <div className="w-16 h-20 bg-secondary-bg border border-white/5 rounded-lg overflow-hidden flex-shrink-0">
                      <img src={item.image} alt={item.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    </div>
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div>
                        <h4 className="text-[0.65rem] font-black tracking-widest uppercase mb-1">{item.name}</h4>
                        <div className="flex gap-3 items-center opacity-40 text-[0.5rem] font-black uppercase tracking-widest">
                          <span>Size: {item.size}</span>
                          <span className="w-1 h-1 bg-white/50 rounded-full"></span>
                          <span>Qty: {item.quantity}</span>
                        </div>
                      </div>
                      <p className="text-[0.7rem] font-bold italic">₹{item.price * item.quantity}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="pt-8 border-t border-white/10 space-y-4">
                <div className="flex justify-between items-center text-[0.65rem] font-black tracking-[0.2em] uppercase opacity-40">
                  <span>Subtotal</span>
                  <span>₹{subtotal}.00</span>
                </div>
                <div className="flex justify-between items-center text-[0.65rem] font-black tracking-[0.2em] uppercase">
                  <span className="opacity-40">Shipping</span>
                  <span className="text-accent underline underline-offset-4 decoration-accent/30 font-bold">EXPRESS // FREE</span>
                </div>
                <div className="pt-4 flex justify-between items-end">
                  <p className="text-[0.5rem] font-black text-gray-500 tracking-[0.3em] uppercase">Total Assessment</p>
                  <p className="text-3xl font-black italic tracking-tighter">₹{subtotal}.00</p>
                </div>
              </div>
            </div>

            <div className="bg-accent/5 border border-accent/20 rounded-2xl p-6 flex gap-5">
              <div className="w-10 h-10 border border-accent/20 rounded-full flex items-center justify-center text-accent flex-shrink-0">
                <Star size={16} fill="currentColor" className="opacity-40" />
              </div>
              <div className="space-y-1">
                <h4 className="text-[0.6rem] font-black tracking-widest text-accent uppercase">Original Promise</h4>
                <p className="text-[0.55rem] font-bold text-accent/60 leading-relaxed uppercase tracking-widest">
                  Authentication guaranteed with every piece. Engineered for durability, curated for the avant-garde.
                </p>
              </div>
            </div>
          </div>
...
export default function App() {
  ...
  const handleLogin = async () => {
    ...
    // Phase 2: Profile Sync (don't block login if sync fails, but log it)
    if (userFromPopup) {
      const userPath = `users/${userFromPopup.uid}`;
      try {
        const userRef = doc(db, 'users', userFromPopup.uid);
        const userDoc = await getDoc(userRef);
        
        const userData: any = {
          uid: userFromPopup.uid,
          email: userFromPopup.email,
          displayName: userFromPopup.displayName,
          photoURL: userFromPopup.photoURL,
        ...
        // We use the helper for system diagnosis but we've already logged in
        try {
          handleFirestoreError(error, OperationType.WRITE, userPath);
        } catch (innerError) {
          // Inner error is the JSON string, we can set a different minor error if we want
          // but let's not show the "Auth Error" overlay for a background sync issue
          // unless the user really needs to know.
        }
      }
    }
  };
      ...
      <div className="min-h-screen bg-dark-bg flex items-center justify-center">
        ...
        >
          <div className="flex flex-col items-center -space-y-1 md:-space-y-2">
            <div className="text-2xl md:text-5xl font-logo font-extrabold tracking-[-0.05em] text-white accent-glow-white">ORIGINAL</div>
            <div className="text-[0.45rem] md:text-[0.65rem] font-black tracking-[0.5em] md:tracking-[0.8em] text-accent uppercase opacity-80 pl-2">Archives</div>
          </div>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 bg-dark-bg text-white`}>
      ...
      <AnimatePresence>
        ...
        <Routes>
          ...
          <Route path="/checkout" element={
            <CheckoutPage 
              ...
              onConfirmOrder={confirmOrderPlacement}
              pendingShippingDetails={pendingShippingDetails}
            />
          } />
        </Routes>
        ...
        {/* Checkout Modal removed - now a page route */}
      </div>
  );
}
